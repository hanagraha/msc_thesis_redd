...
# # Define function to homogenize df shapes (needed for plotting)
# def homogenize_dfs(df_list):
    
#     # Create copy of dataframe
#     dflist_copy = [df.copy() for df in df_list]
    
#     # Extract largest cluster size over all years
#     max_columns = max([df.shape[1] for df in dflist_copy])
    
#     # Iterate over each dataframe
#     for i, df in enumerate(dflist_copy):
        
#         # Get number of columns in dataframe
#         current_columns = df.shape[1]
        
#         # If statement if columns are missing
#         if current_columns < max_columns:
            
#             # Add missing columns with frequency 0
#             for col in range(current_columns, max_columns):
#                 df[col] = 0
                
#     return dflist_copy

# # Homogenize shape of gfc dataframes
# gfc_homo_atts = homogenize_dfs(gfc_forclust_atts)

# # Homogenize shape of tmf dataframes
# tmf_homo_atts = homogenize_dfs(tmf_forclust_atts)



# def agree_patch_lab(arr_list, yearrange, nodata_val):
...
#         # Create mask where pixels are equal to 8 (valid pixels to label)
#         mask = (arr == 8)
...
# # Cluster spatial agreement layers (183.933676 seconds)
# agree_forclust = patch_label([agreement_arrs[0]], [2013], nodata_val, agree=True)
